...
/** Slim top-hat DIN rail — sits behind module clips, never through bodies */
export function DinRail({ materials }: Props) {
  const length = BOARD.width - 0.55;
  const y = BOARD.railY;
  const z = BOARD.railZ;
  ...
  return (
    <group position={[0, y, z]}>
      ...
      <mesh castShadow material={materials.dinRail} position={[0, 0, -0.035]}>
        <boxGeometry args={[length, 0.014, 0.045]} />
      </mesh>
      {/* Top flange */}
      <mesh position={[0, 0.032, 0.012]} castShadow material={materials.dinRail}>
        <boxGeometry args={[length, 0.028, 0.028]} />
      </mesh>
      {/* Bottom flange */}
      <mesh position={[0, -0.032, 0.012]} castShadow material={materials.dinRail}>
        <boxGeometry args={[length, 0.028, 0.028]} />
